...
int main (int argc, char *argv[])
{
	...
	btn = gtk_button_new_with_label (_("Shutdown"));
	...
	btn = gtk_button_new_with_label (_("Reboot"));
    gtk_widget_size_request (btn, &req);
    if (req.width < MIN_WIDTH) gtk_widget_set_size_request (box, MIN_WIDTH, -1);
	...
	if (strlen (buffer))
	    btn = gtk_button_new_with_label (_("Logout"));
	else
	    btn = gtk_button_new_with_label (_("Exit to command line"));
    gtk_widget_size_request (btn, &req);
    if (req.width < MIN_WIDTH) gtk_widget_set_size_request (box, MIN_WIDTH, -1);
